"""
CoCoLasso (Convex Corrected Lasso) Python Implementation

High-dimensional error-in-variables regression with support for:
- Additive error in covariates
- Missing data in covariates
- Block-Descent CoCoLasso (BD-CoCoLasso)
- Three-block generalization (mixed additive + missing error)

Based on: https://arxiv.org/pdf/1510.07123.pdf
"""
...
def l1_proj(v: np.ndarray, b: float) -> np.ndarray:
    """
    Efficient projection onto the L1 ball of radius b.
    Reference: Duchi et al. (2008), Efficient Projections onto the L1-Ball
    for Learning in High Dimensions, ICML.

    Parameters
    ----------
    v : array, vector to project
    b : float, radius of the L1 ball (must be > 0)

    Returns
    -------
    w : array, projected vector
    """
...
def admm_proj(
    ...
    """
    ADMM algorithm to find the nearest positive semi-definite matrix
    with respect to the max norm.

    Solves: min_{R,S} ||R - mat||_max  s.t.  R >= 0, R - S = mat, ||S||_1 <= mu/2

    Parameters
    ----------
    mat : (p, p) array, matrix to project
    epsilon : float, approximation threshold for PSD space
    mu : float, penalty parameter for ADMM
    it_max : int, maximum number of iterations
    etol : float, tolerance for convergence of primal and dual residual
    etol_distance : float, tolerance for convergence of the distance

    Returns
    -------
    dict with keys:
        'mat': projected PSD matrix
        'df_ADMM': dict of convergence diagnostics
    """
    p = mat.shape[0]
    ...
    while itr < it_max:
        ...
        M_lower = M[lower_idx]
        S_lower = M_lower - l1_proj(M_lower, mu / 2)
        S = np.zeros((p, p))
        S[lower_idx] = S_lower
...
def hm_proj(
    ...
    """
    HM-lasso algorithm to find the nearest positive semi-definite matrix
    with respect to the Frobenius norm.

    Parameters
    ----------
    sigma_hat : (p, p) array, covariance matrix to project
    R : (p, p) array or None, weight matrix
    a : float, hmlasso (1) or cocolasso (0)
    iter_max : int, maximum number of iterations
    epsilon : float, stopping criterion
    mu : float, penalty parameter
    tolerance : float, tolerance for convergence
    norm : str, 'F' for Frobenius or 'max' for max norm

    Returns
    -------
    Ak : (p, p) array, projected PSD matrix
    """
    n = sigma_hat.shape[0]
...
def _scad_weight(beta_j: float, lambda_val: float, a: float = 3.7) -> float:
    """
    Compute SCAD penalty weight for a given coefficient value.
...
def lasso_covariance(
    ...
    """
    Solve the Lasso in covariance form:
        min_beta  (1/2) beta' XX beta - Xy' beta + lambda * ||beta||_1

    Uses coordinate descent with optional SCAD adaptive weights.
    ...
    XX : (p, p) array, covariance matrix (Sigma)
    Xy : (p,) or (p,1) array, cross-covariance vector (rho)
    beta_start : (p,) array, initial beta values
    penalty : str, 'lasso' or 'SCAD'
    max_iter : int, maximum iterations
    opt_tol : float, convergence tolerance
    zero_threshold : float, threshold for zeroing small coefficients

    Returns
    -------
    dict with keys:
        'coefficients': final beta vector
        'num_it': number of iterations performed
    """
...
def lasso_covariance_block(
    ...
    """
    Lasso in covariance form for BD-CoCoLasso (2-block version).

    Alternating block coordinate descent:
      - Block 1 (uncorrupted): standard Lasso
      - Block 2 (corrupted): CoCoLasso-corrected Lasso

    Parameters
    ----------
    n : int, number of samples
    p1 : int, number of uncorrupted predictors
    p2 : int, number of corrupted predictors
    X1 : (n, p1) array, uncorrupted design matrix
    Z2 : (n, p2) array, corrupted design matrix
    y : (n,) array, response vector
    sigma1 : (p1, p1) array, covariance for X1
    sigma2 : (p2, p2) array, corrected PSD covariance for Z2
    lambda_val : float, penalty parameter
    noise : str, 'additive' or 'missing'
    ratio_matrix : (p2, p2) array or None, observation ratio matrix for missing data
    beta1_start : (p1,) array, initial beta1
    beta2_start : (p2,) array, initial beta2
    penalty : str, 'lasso' or 'SCAD'
    max_iter : int, maximum iterations
    opt_tol : float, convergence tolerance
    zero_threshold : float, threshold for zeroing small coefficients
    ...
    -------
    dict with keys:
        'coefficients_beta1': final beta1
        'coefficients_beta2': final beta2
...
def lasso_covariance_block_general(
    n: int,
    p1: int,
    p2: int,
    p3: int,
    ...
    """
    Lasso in covariance form for Three-block BD-CoCoLasso.

    Three blocks:
      - Block 1 (uncorrupted): standard Lasso
      - Block 2 (additive error): CoCoLasso-corrected Lasso
      - Block 3 (missing data): CoCoLasso-corrected Lasso

    Parameters
    ----------
    n : int, number of samples
    p1 : int, number of uncorrupted predictors
    p2 : int, number of corrupted predictors with additive error
    p3 : int, number of corrupted predictors with missing data
    X1 : (n, p1) array or None, uncorrupted design matrix
    Z2 : (n, p2) array, corrupted design matrix (additive error)
    Z3 : (n, p3) array, corrupted design matrix (missing data)
    y : (n,) array, response vector
    sigma1 : (p1, p1) array or None, covariance for X1
    sigma2 : (p2, p2) array, corrected PSD covariance for Z2
    sigma3 : (p3, p3) array, corrected PSD covariance for Z3
    lambda_val : float, penalty parameter
    ratio_matrix : (p3, p3) array or None, observation ratio matrix for missing data
    beta1_start, beta2_start, beta3_start : initial coefficient vectors
    penalty : str, 'lasso' or 'SCAD'
    max_iter, opt_tol, zero_threshold : optimization parameters

    Returns
    -------
    dict with keys:
        'coefficients_beta1', 'coefficients_beta2', 'coefficients_beta3', 'num_it'
    """
    y = y.ravel()
...
def _compute_ratio_matrix(Z: np.ndarray, p: Optional[int] = None, offset: int = 0) -> np.ndarray:
    """
    Compute the observation ratio matrix for missing data setting.

    R_{jk} = n_{jk} / n, where n_{jk} is the number of rows where both
    features j and k are observed (non-NA).

    Parameters
    ----------
    Z : (n, p2) array, corrupted design matrix with potential NaN values
    p : int or None, number of features (columns of Z to use)
    offset : int, column offset in Z

    Returns
    -------
    ratio_matrix : (p2, p2) array
    """
    if p is None:
        p = Z.shape[1]
    n = Z.shape[0]
...
def _log_space(start: float, stop: float, num: int) -> np.ndarray:
    """Generate logarithmically spaced sequence (like R's lseq)."""
...
def _corrected_covariance_multiplicative(mat: np.ndarray, y: np.ndarray,
                                          ...
                                          tau: float) -> tuple:
    """
    Compute corrected covariance and cross-covariance for multiplicative error.

    For Z = X * M where log(M_ij) ~ N(0, tau^2):
    ...
    Returns
    -------
    (Gamma, rho_tilde) : corrected covariance matrix and cross-covariance vector
    """
...
def _lambda_max(Z: np.ndarray, y: np.ndarray, n: int,
    ...
    """
    Compute the maximum lambda value (smallest lambda that sets all coefficients to zero).

    For additive noise:       lambda_max = max|rho_tilde| where rho_tilde = (1/n) Z'y
    For missing data:         lambda_max = max|rho_tilde| where rho_tilde = (1/n) Z'y / diag(R)
    For multiplicative noise: lambda_max = max|rho_tilde| where rho_tilde = (1/n) Z'y / exp(tau^2/2)
    """
    y = y.ravel()
    if noise == "additive":
        rho_tilde = (1 / n) * Z.T @ y
...
def _lambda_max_block(Z: np.ndarray, y: np.ndarray, n: int, p1: int, p2: int,
    ...
    """Compute lambda_max for the block descent setting."""
    y = y.ravel()
    X1 = Z[:, :p1]
    Z2 = Z[:, p1:]
    if noise == "additive":
...
def cv_covariance_matrices(
    ...
    """
    Create projected PSD covariance matrices for cross-validation in CoCoLasso.

    For each fold k:
      - Compute the corrected covariance on the training set
      - Project to nearest PSD matrix
      - Compute the corrected covariance on the test set

    Parameters
    ----------
    K : int, number of CV folds
    mat : (n, p) array, design matrix
    y : (n,) array, response vector
    p : int, number of predictors
    mu : float, ADMM penalty parameter
    tau : float or None, standard deviation of additive error
    ratio_matrix : (p, p) array or None, observation ratio matrix
    etol : float, ADMM tolerance
    noise : str, 'additive', 'missing', or 'multiplicative'
    mode : str, 'ADMM' or 'HM'
    ...
    def _project(cov_mat, R=None):
        if mode == "ADMM":
            return admm_proj(cov_mat, mu=mu, etol=etol)["mat"]
        else:
            return hm_proj(cov_mat, R=R, mu=mu, tolerance=etol)
...
def cv_covariance_matrices_block(
    K: int,
    mat: np.ndarray,
    y: np.ndarray,
    p: int,
    ...
    """
    Create projected PSD covariance matrices for cross-validation in BD-CoCoLasso.

    Parameters
    ----------
    K : int, number of CV folds
    mat : (n, p) array, design matrix (first p1 cols uncorrupted, last p2 cols corrupted)
    y : (n,) array, response vector
    ...
    mu : float, ADMM penalty parameter
    tau : float or None, standard deviation of additive error
    ratio_matrix : (p2, p2) array or None, observation ratio matrix
    etol : float, ADMM tolerance
    noise : str, 'additive' or 'missing'
    mode : str, 'ADMM' or 'HM'
...
def cv_covariance_matrices_block_general(
    K: int,
    mat: np.ndarray,
    y: np.ndarray,
    p: int,
    ...
    """
    Create projected PSD covariance matrices for Three-block BD-CoCoLasso cross-validation.
    ...
    mat : (n, p) array, design matrix
    y : (n,) array, response vector
    p : int, total number of predictors (p1 + p2 + p3)
    p1 : int, number of uncorrupted predictors
    p2 : int, number of corrupted predictors with additive error
    p3 : int, number of corrupted predictors with missing data
    mu : float, ADMM penalty parameter
    tau : float or None, standard deviation of additive error
    ratio_matrix : (p3, p3) array or None, observation ratio matrix
    etol : float, ADMM tolerance
    mode : str, 'ADMM' or 'HM'

    Returns
    -------
    dict with projected matrices for each fold
    """
    n = mat.shape[0]
    n_without_fold = n - n // K
    n_one_fold = n // K
...
def _preprocess_data(
    ...
    """
    Preprocess the data: center and scale Z and y, handle missing values.

    For missing data: center columns ignoring NAs, replace NAs with 0, then scale.
    For additive error: standard centering and scaling.
...
def pathwise_coordinate_descent(
    ...
    """
    Pathwise coordinate descent for CoCoLasso.

    Implements the CoCoLasso algorithm with K-fold cross-validation
    to select the optimal regularization parameter lambda.

    Parameters
    ----------
    Z : (n, p) array, corrupted design matrix
    y : (n,) array, response vector
    n : int, number of samples
    p : int, number of features
    center_Z : bool, whether to center Z
    scale_Z : bool, whether to scale Z
    center_y : bool, whether to center y
    scale_y : bool, whether to scale y
    lambda_factor : float, ratio lambda_min/lambda_max
    step : int, number of lambda values to test
    K : int, number of CV folds
    mu : float, ADMM penalty parameter
    tau : float or None, standard deviation of additive error
    etol : float, ADMM tolerance
    optTol : float, convergence tolerance for pathwise descent
    earlyStopping_max : int, max iterations with increasing error
    noise : str, 'additive', 'missing', or 'multiplicative'
    penalty : str, 'lasso' or 'SCAD'
    mode : str, 'ADMM' or 'HM'
...
def blockwise_coordinate_descent(
    ...
    """
    Blockwise coordinate descent for BD-CoCoLasso.

    Implements the Block-Descent CoCoLasso algorithm with K-fold
    cross-validation. The design matrix Z must have the first p1 columns
    as uncorrupted features and the last p2 columns as corrupted features.

    Parameters
    ----------
    Z : (n, p) array, design matrix (first p1 cols uncorrupted, last p2 cols corrupted)
    y : (n,) array, response vector
    n : int, number of samples
    p : int, total number of features (p1 + p2)
    p1 : int, number of uncorrupted predictors
    p2 : int, number of corrupted predictors
    center_Z, scale_Z, center_y, scale_y : bool, preprocessing flags
    lambda_factor : float, ratio lambda_min/lambda_max
    step : int, number of lambda values to test
    K : int, number of CV folds
    mu : float, ADMM penalty parameter
    tau : float or None, standard deviation of additive error
    etol : float, ADMM tolerance
    optTol : float, convergence tolerance
    earlyStopping_max : int, max iterations with increasing error
    noise : str, 'additive' or 'missing'
    penalty : str, 'lasso' or 'SCAD'
    mode : str, 'ADMM' or 'HM'
    ...
    output = cv_covariance_matrices_block(
...
def coco(
    ...
    """
    Main entry point for CoCoLasso / BD-CoCoLasso.

    If block=True, uses blockwise_coordinate_descent (BD-CoCoLasso).
    If block=False, uses pathwise_coordinate_descent (CoCoLasso).
    ...
    n : int, number of samples
    p : int, number of features
    p1 : int or None, number of uncorrupted predictors (required if block=True)
    p2 : int or None, number of corrupted predictors (required if block=True)
    center_Z, scale_Z, center_y, scale_y : bool, preprocessing flags
    lambda_factor : float, ratio lambda_min/lambda_max
    step : int, number of lambda values to test
    K : int, number of CV folds
    ...
    earlyStopping_max : int, max iterations with increasing error
    noise : str, 'additive', 'missing', or 'multiplicative'
    block : bool, use BD-CoCoLasso (True) or CoCoLasso (False)
    penalty : str, 'lasso' or 'SCAD'
    mode : str, 'ADMM' or 'HM'
...
def generalcoco(
    ...
    """
    Three-block BD-CoCoLasso for mixed additive error + missing data.

    Wrapper around blockwise_coordinate_descent_general.

    Parameters
    ----------
    Z : (n, p) array, design matrix
    y : (n,) array, response vector
    n : int, number of samples
    p : int, total number of features (p1 + p2 + p3)
    p1 : int, number of uncorrupted predictors
    p2 : int, number of corrupted predictors with additive error
    p3 : int, number of corrupted predictors with missing data
    center_Z, scale_Z, center_y, scale_y : bool, preprocessing flags
    lambda_factor : float, ratio lambda_min/lambda_max
    step : int, number of lambda values
    K : int, number of CV folds
    mu : float, ADMM penalty parameter
    tau : float or None, standard deviation of additive error
    etol : float, ADMM tolerance
    ...
    dict with keys:
        ...
        'mean_Z', 'sd_Z', 'mean_y', 'sd_y'
    """
    return blockwise_coordinate_descent_general(
...
def cov_autoregressive(p: int, rho: float = 0.5) -> np.ndarray:
    """
    Create autoregressive covariance matrix.

    Parameters
    ----------
    p : int, number of features
    rho : float, autoregressive parameter (default 0.5)

    Returns
    -------
    cov : (p, p) array, autoregressive covariance matrix
    """
    cov = np.zeros((p, p))
    for i in range(p):
        for j in range(p):
...
def simulate_data(
    ...
    """
    Simulate data for testing CoCoLasso.

    Parameters
    ----------
    n : int, number of samples
    p : int, number of features
    beta : (p,) array or None, true coefficient vector
    noise : str, 'additive' or 'missing'
    tau : float, standard deviation of additive error
    missing_rate : float, proportion of missing values (for missing data setting)
    seed : int or None, random seed

    Returns
    -------
    dict with keys:
        'Z': corrupted design matrix
        'y': response vector
        'X': true design matrix (before corruption)
    ...
    cov = cov_autoregressive(p)
    X = np.random.multivariate_normal(np.zeros(p), cov, size=n)
    y = X @ beta + np.random.normal(0, 2, size=n)

    if noise == "additive":
        W = np.random.normal(0, tau, size=(n, p))
        Z = X + W
    elif noise == "missing":
        Z = X.copy()
        mask = np.random.random((n, p)) < missing_rate
        Z[mask] = np.nan
    else:
        raise ValueError(f"Unknown noise type: {noise}")

    return {"Z": Z, "y": y, "X": X, "beta": beta}


def blockwise_coordinate_descent_general(
    Z: np.ndarray,
    y: np.ndarray,
    n: int,
    p: int,
    ...
    The design matrix Z must be organized as:
      - Columns 0..p1-1: uncorrupted features
      - Columns p1..p1+p2-1: features with additive error
      - Columns p1+p2..p-1: features with missing data
    ...
    output = cv_covariance_matrices_block_general(
    ...
    for i in range(step):
        ...
        for k in range(K):
            ...
            out = lasso_covariance_block_general(
