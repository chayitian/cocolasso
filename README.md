...
**CoCoLasso**（Convex Corrected Lasso，凸修正Lasso）是一种高维误差变量回归算法，用于处理协变量中存在 **加性误差（additive error）** 或 **缺失数据（missing data）** 的情况。该算法由 Datta & Zou (2017) 提出，核心思想是对被噪声污染的协方差矩阵进行 **正半定投影（PSD projection）** 修正，然后基于修正后的协方差矩阵进行 Lasso 回归。本实现的标准 CoCoLasso 还支持乘性误差校正。
...
在乘性误差设定 $Z = X \odot M$，且 $\log(M_{ij}) \sim \mathcal{N}(0, \tau^2)$ 时：
...
当前仓库未提供 `requirements.txt` 或 `pyproject.toml`。直接运行代码至少需要：
...
6. **求解器限制**：`solver="sklearn"` 仅支持 `penalty="lasso"`；使用 SCAD 时应保持默认 `solver="coordinate_descent"`。
...
结果默认保存为 `results/simulation_results.csv`，包含各场景下 C、IC、SE、PE 的中位数及 Bootstrap 标准误。已归档的最新结果见 `results/results_20260515/simulation_results_table.md`；该归档结果对应旧实现和 Git 提交 `9fb0e47`，核心算法修改后需重新运行复现实验更新结果。
