# CoCoLasso / BDCoCoLasso

参考 BDCoCoLasso 作者的 R 语言包实现 Python 版本的 CoCoLasso、BDCoCoLasso，并扩展到三块混合场景的 GeneralCoCoLasso。当前主线聚焦 CoCoLasso 相关实现、复现实验脚本和结果记录。
...
**CoCoLasso**（Convex Conditioned Lasso，凸条件化 Lasso）是一种高维误差变量回归算法，用于处理协变量中存在 **加性误差（additive error）**、**乘性误差（multiplicative error）** 或 **缺失数据（missing data）** 的情况。该算法由 Datta & Zou (2017) 提出，核心思想是先构造被污染数据下的无偏或校正 Gram 矩阵和交叉矩，再把可能非半正定的 Gram 矩阵投影到 **正半定（PSD）** 锥上，最后求解凸 Lasso 问题。

本文档中“标准 CoCoLasso”指 Datta & Zou (2017) PDF 中的 CoCoLasso 主线；`BDCoCoLasso`、`GeneralCoCoLasso`、`SCAD` 惩罚、`HM` 投影和 `sklearn` 伪数据求解器均为本仓库的工程扩展或兼容实现，不属于该 PDF 的标准算法陈述。
...
$$Z = X \odot M, \quad \log(M_{ij}) \sim \mathcal{N}(0, \tau^2)$$

其中 $\odot$ 表示逐元素乘法。当前实现中，乘性误差仅由标准 `CoCoLasso` / `coco(block=False)` 支持。
...
在论文的一般加性误差设定下，若误差协方差为已知矩阵 $\Sigma_A$，则应从 $Z^\top Z/n$ 中减去 $\Sigma_A$。当前代码实现的是常用特例 $\Sigma_A=\tau^2 I_p$；若启用标准化，内部会按每列标准差换算为对角校正。

在 $\Sigma_A=\tau^2 I_p$ 的特例下：
...
论文第 4.2 节给出一般乘性误差形式，可用 $\mu_M$ 和 $\Sigma_M$ 构造校正矩阵。当前代码实现的是论文数值实验使用的 iid log-normal 特例：$Z = X \odot M$ 且 $\log(M_{ij}) \sim \mathcal{N}(0, \tau^2)$。在该特例下：

$$\tilde{\Sigma}_{jk} = \frac{\hat{\Sigma}_{jk}}{e^{\tau^2}}, \quad j \neq k$$

$$\tilde{\Sigma}_{jj} = \frac{\hat{\Sigma}_{jj}}{e^{2\tau^2}}, \quad \tilde{\rho}_j = \frac{\hat{\rho}_j}{e^{\tau^2/2}}$$

当前实现中，乘性误差修正用于标准 CoCoLasso 路径，不用于 BDCoCoLasso 或 GeneralCoCoLasso。

### 3.2 正半定投影

修正后的 $\tilde{\Sigma}$ **不一定是正半定矩阵**，而 Lasso 的协方差形式要求协方差矩阵正半定以保证凸性。因此需要对 $\tilde{\Sigma}$ 进行投影：

$$\Sigma^{\text{PSD}} = \Pi_{\text{PSD}}(\tilde{\Sigma}) = \arg\min_{R \succeq 0} \|R - \tilde{\Sigma}\|_{\max}$$

Datta & Zou (2017) PDF 中使用的是最大范数意义下的 PSD 投影。本实现另外提供 HM/Frobenius 投影作为工程选项；使用该选项时不再是 PDF 中的原始投影形式。
...
本实现使用固定的 $\mu$，可通过参数 `mu` 调整 ADMM 惩罚强度。

#### HM-Lasso 投影（Frobenius 范数，扩展选项）
...
PDF 中的 CoCoLasso 使用 Lasso 的 $\ell_1$ 凸惩罚。`SCAD` 是本仓库提供的非凸惩罚扩展，启用后不再对应论文中的凸 CoCoLasso 目标。
...
1. 将数据随机分为 K 折；若传入 `random_state`，折分由该种子控制
2. 对每个 $\lambda$ 值和每折 $k$：
...
└── _utils.py                # 共享工具函数（内部模块）
    ...
    ├── _lasso_sklearn()                  # Cholesky 转换 + sklearn Lasso 求解器
...
该函数是非 PDF 标准内容的扩展投影选项。论文中的 CoCoLasso 理论对应最大范数 PSD 投影，而不是 Frobenius 投影。
...
### 6.4 `_lasso_covariance(n, p, lambda_val, XX, Xy, beta_start, penalty)`
...
### 6.12 `_lasso_sklearn(n, p, lambda_val, XX, Xy, beta_start)`

**功能**：将协方差形式 Lasso 转换为伪数据矩阵形式，并调用 `sklearn.linear_model.Lasso` 求解。

**转换方式**：对 PSD 矩阵 $XX$ 做 Cholesky 分解 $XX = U^\top U$，构造 $\tilde{W} = \sqrt{p}U$ 与 $\tilde{Y} = U^{-\top}\sqrt{p}Xy$，再调用 sklearn Lasso。

**限制**：仅支持 `penalty="lasso"`，不支持 SCAD 惩罚。若 Cholesky 分解失败或输入含 NaN/Inf，会返回零向量以保证流程不中断。
...
当前仓库提供了 `requirements.txt`。直接运行代码至少需要：
...
# ---- 标准 CoCoLasso ----
model = CoCoLasso(tau=0.75, noise="additive", cv_folds=4, random_state=42)
model.fit(Z, y)
...
# ---- 使用 sklearn 求解器加速（仅支持 lasso 惩罚）----
model = CoCoLasso(tau=0.75, noise="additive", solver="sklearn")
model.fit(Z, y)
...
# 标准 CoCoLasso
result = coco(Z=Z, y=y, n=n, p=p, step=100, K=4, mu=1.0,
...
result = coco(Z=Z, y=y, n=n, p=p, p1=50, p2=200, step=100, K=4,
              mu=1.0, tau=0.3, noise="additive", block=True)

# 三块 BD-CoCoLasso
result = generalcoco(Z=Z, y=y, n=n, p=p, p1=20, p2=100, p3=130,
                     step=100, K=4, mu=1.0, tau=0.3)
```
...
SCAD 仅由默认的 `solver="coordinate_descent"` 支持；`solver="sklearn"` 仅支持 `penalty="lasso"`。

### 7.5 测试现状

当前仓库只包含 `tests/测试分析报告.md`，未包含可直接运行的 `test_*.py` 或 `run_all_tests.py` 测试脚本。若后续恢复测试代码，需要同步更新本 README 中的测试命令。
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `alpha` | float or None | None | 固定正则化强度；为 `None` 时沿 lambda 路径做交叉验证自动选择 |
...
| `mu` | float | 1.0 | ADMM 惩罚参数 |
| `max_iter` | int | 100 | lambda 路径上的最大搜索步数 |
| `cv_folds` | int | 4 | 交叉验证折数 |
| `random_state` | int or None | None | 交叉验证折分随机种子；None 时沿用全局 numpy 随机状态 |
| `tol` | float | 1e-5 | 收敛容差 |
...
| `solver` | str | "coordinate_descent" | Lasso 求解器：`"coordinate_descent"` 或 `"sklearn"` |

`BDCoCoLasso` 和 `GeneralCoCoLasso` 也使用上述通用参数；下方只列出各自额外需要的块维度参数。
...
`BDCoCoLasso` 要求 `p1 + p2 == Z.shape[1]`，并且仅支持 `noise="additive"` 或 `noise="missing"`。
...
`GeneralCoCoLasso` 要求 `p1 + p2 + p3 == Z.shape[1]`，列顺序固定为未污染块、加性误差块、缺失数据块；也支持 `p1=0` 的两块混合场景。

### 8.4 估计器属性

| 属性 | 类型 | 说明 |
|------|------|------|
| `coef_` | ndarray (p,) | 原始特征尺度下的最优系数向量 |
| `intercept_` | float | 截距 |
| `coef_sd_` | ndarray (p,) | 原始特征尺度下 1-std 准则对应的系数向量 |
| `coef_scaled_` | ndarray (p,) | 内部预处理尺度下的最优系数向量 |
| `coef_sd_scaled_` | ndarray (p,) | 内部预处理尺度下 1-std 准则对应的系数向量 |
| `lambda_opt_` | float | 交叉验证选出的最优 lambda |
| `lambda_sd_` | float | 1-std 准则下的 lambda |
| `cv_results_` | dict | 交叉验证误差详情 |
| `coef_path_` | dict | 原始特征尺度下的系数路径 |
| `coef_path_scaled_` | dict | 内部预处理尺度下的系数路径 |
| `n_iter_` | int | 实际迭代步数 |
...
| `K` | int | 4 | 交叉验证折数 |
| `mu` | float | 1.0 | ADMM 惩罚参数 |
| `tau` | float | None | 加性/乘性误差标准差 |
| `etol` | float | 1e-4 | ADMM 收敛容差 |
| `optTol` | float | 1e-5 | 优化收敛容差 |
| `earlyStopping_max` | int | 10 | 误差上升时的最大迭代次数 |
| `noise` | str | "additive" | `block=False` 时支持 `"additive"`、`"missing"`、`"multiplicative"`；`block=True` 时支持 `"additive"`、`"missing"` |
...
`generalcoco()` 也支持 `solver="coordinate_descent"` 或 `solver="sklearn"`；其中 `solver="sklearn"` 仅适用于 `penalty="lasso"`。
...
函数式接口返回的 `beta_opt`、`beta_sd` 和 `data_beta` 保持为内部预处理尺度；若要和原始尺度真值系数比较，需要用 `beta * sd_y / sd_Z` 还原。

### 8.8 重要注意事项

1. **加性误差设定**：必须提供 `tau` 参数（加性误差的标准差）。论文默认所有变量已中心化；代码默认 `center_Z=True`、`center_y=True`，并在标准化后同步调整加性误差方差。
2. **缺失数据设定**：`tau=None`。建议设置 `center_Z=True`，先将缺失值排除后中心化，再将缺失位置0。
3. **噪声类型支持**：`CoCoLasso` 支持加性、缺失和乘性误差；`BDCoCoLasso` 支持加性和缺失误差；`GeneralCoCoLasso` 支持加性误差块 + 缺失数据块的混合场景。
4. **块维度约束**：`BDCoCoLasso` 要求 `p1 + p2 == p`；`GeneralCoCoLasso` 要求 `p1 + p2 + p3 == p`。
5. **列顺序约定**：BD-CoCoLasso 的列顺序为未污染 → 污染；三块设定的列顺序为未污染 → 加性误差 → 缺失数据。
6. **求解器限制**：`solver="sklearn"` 仅支持 `penalty="lasso"`；使用 SCAD 时应保持默认 `solver="coordinate_descent"`。SCAD 为非凸扩展，不属于 PDF 中的标准 CoCoLasso。
7. **缺失率过高**：缺失率超过 0.7 可能导致算法失败（特征值分解出现无穷大或 NaN）。
8. **计算开销**：BD-CoCoLasso 和 GeneralCoCoLasso 在高维场景下计算开销较大，可通过调整 `solver`、`mu`、降低 `etol` 或 `optTol`、减小 `earlyStopping_max` 来加速。
9. **可复现性**：`random_state` 只控制交叉验证折分，不改变 CoCoLasso 的目标函数、协方差校正、PSD 投影或 calibrated CV 公式。复现实验脚本会额外固定 bootstrap 标准误的随机种子。
...
基于 CoCoLasso 论文 (Datta & Zou, 2017) 的模拟设置，当前主线复现实验脚本覆盖 **CoCoLasso** 在加性测量误差和乘性测量误差场景下的表现。
...
当前主线代码未包含该部分的已验证实现；以下内容仅作为理解原论文对照表的背景参考。
...
所有指标报告中位数，标准误通过 500 次 Bootstrap 自助法计算。当前复现实验脚本中，每次蒙特卡洛的 CV 折分使用 `random_state=seed + 20000`，bootstrap 标准误使用按场景和指标固定的局部随机种子。
...
```bash
# 完整实验（100次蒙特卡洛，所有场景）
python reproduce/cocolasso_simulation.py --mode full
...
结果默认保存为 `results/simulation_results.csv`，包含各场景下 C、IC、SE、PE 的中位数及 Bootstrap 标准误。已归档的最新结果见 `results/results_20260520/simulation_results_table.md`；核心算法、预处理、协方差校正或求解器修改后，需要重新运行复现实验并更新结果说明。
...
建议先用 `--mode quick` 验证流程，再逐步运行完整实验。实际耗时会随硬件、`solver`、PSD 投影模式和随机折分而变化。

---

## 10. 参考文献

【1】Datta A, Zou H. CoCoLasso for High-dimensional Error-in-variables Regression[J]. Annals of Statistics, 2017, 45(6):2400-2426. [arXiv:1510.07123](https://arxiv.org/pdf/1510.07123.pdf)
