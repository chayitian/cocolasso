...
参考 BDCoCoLasso 作者的 R 语言包实现 Python 的 CoCoLasso 并进行复现，之后拓展到 AdaCoCoLasso（自适应CoCoLasso）并进行复现。
...
├── cocolasso.py             # sklearn 风格估计器（主入口）
│   ├── CoCoLasso            # 标准 CoCoLasso 估计器
│   ├── BDCoCoLasso          # 二块下降 CoCoLasso 估计器
│   └── GeneralCoCoLasso     # 三块混合 CoCoLasso 估计器
└── _core.py                 # 核心算法实现（内部模块）
    ├── 投影与惩罚
    │   ├── _l1_proj()                        # L1 球投影
    ...
    ├── 主算法
    │   ├── _pathwise_coordinate_descent()          # CoCoLasso
    │   ├── _blockwise_coordinate_descent()         # BD-CoCoLasso
    │   └── _blockwise_coordinate_descent_general() # 三块 BD-CoCoLasso
    └── 向后兼容接口
        ├── coco()                           # CoCoLasso / BD-CoCoLasso 函数式入口
        └── generalcoco()                    # 三块 BD-CoCoLasso 函数式入口
```
...
```python
from cocolasso import CoCoLasso, BDCoCoLasso, GeneralCoCoLasso
import numpy as np
...
```python
from cocolasso import coco, generalcoco
