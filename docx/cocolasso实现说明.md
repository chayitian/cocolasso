...
> 说明：本文主要解释标准 `CoCoLasso` 的算法与实现细节。当前仓库还包含 `BDCoCoLasso`（二块下降）和 `GeneralCoCoLasso`（三块混合）实现；块下降模型的完整 API、列顺序和运行入口以根目录 `README.md` 为准。
...
当前实现的支持边界如下：标准 `CoCoLasso` 支持加性、缺失、乘性三种噪声；`BDCoCoLasso` 支持加性和缺失噪声；`GeneralCoCoLasso` 支持“加性误差块 + 缺失数据块”的三块混合场景，不支持乘性误差块。
...
#### 2.3.2 HM-Lasso 投影（Frobenius 范数）

HM-Lasso（High-dimensional Matrix Lasso）投影求解以下优化问题：
...
| 当前代码入口 | `mode="ADMM"` | `mode="HM"` |
| 适用场景 | 加性、缺失、乘性噪声均可选 | 加性、缺失、乘性噪声均可选；缺失数据场景可利用 R 权重 |
...
$$c_j = \tilde{\rho}_j - \sum_{k \neq j}\hat{\Sigma}_{jk}\beta_k$$

$$\beta_j = \frac{S(c_j, \lambda)}{\hat{\Sigma}_{jj}}$$

其中 S 是软阈值算子：

$$S(z, \lambda) = \operatorname{sign}(z)\max(|z|-\lambda, 0)$$

代码中等价地使用 $S_0=\sum_{k \neq j}\hat{\Sigma}_{jk}\beta_k-\tilde{\rho}_j$，并按 $S_0$ 的正负分支更新 $\beta_j$。
...
**优势：** 利用 sklearn 的高效 C 实现，历史测试中观察到明显加速；具体倍数取决于数据规模、PSD 投影结果和数值条件。支持 warm_start（通过 `lasso.coef_` 初始化）。
...
2. **1-std 准则（lambda_sd）：** 选择满足 CV 误差 ≤ min_error + std_error 且 λ ≥ λ_opt 的最大 λ。这倾向于选择更强正则化（更稀疏）的模型。
...
由于预处理阶段对 Z 和 y 进行了中心化和标准化，如需解释原始变量尺度下的系数，需要进行尺度还原：
...
当前代码中，函数式接口返回的 `beta_opt`、`beta_sd` 仍是算法内部预处理尺度上的系数；sklearn 风格估计器的 `coef_`、`coef_sd_` 和 `coef_path_` 已转换到原始特征尺度。若需要查看估计器内部的预处理尺度系数，可使用 `coef_scaled_`、`coef_sd_scaled_` 和 `coef_path_scaled_`。
...
6.  1-std 准则:
    ...
    lambda_sd = max{lambda: error(lambda) <= threshold 且 lambda >= lambda_opt}
    beta_sd = 对应的系数
...
| `BDCoCoLasso` | 二块下降估计器，要求 `p1 + p2 == p`，支持 `additive`、`missing` |
| `GeneralCoCoLasso` | 三块混合估计器，要求 `p1 + p2 + p3 == p`，列顺序为未污染、加性误差、缺失数据 |
| `coco()` | 函数式接口，`block=False` 调标准 CoCoLasso，`block=True` 调 BDCoCoLasso |
| `generalcoco()` | 三块混合场景的函数式接口 |
...
最新归档结果见 `results/results_20260515/simulation_results_table.md`；该结果对应旧实现和 Git 提交 `9fb0e47`，核心算法修改后需重新运行复现实验更新结果。
